...
export const GeneratePdfModal: React.FC<GeneratePdfModalProps> = ({
  ...
  // Payment defaults
  const amountPaid = lot?.amountPaid ?? draftData?.amountPaid ?? finalFarmerNet;
  const balanceDue = Math.max(0, finalFarmerNet - amountPaid);
  const paymentStatus = balanceDue === 0 ? 'Paid' : amountPaid > 0 ? 'Partial' : 'Unpaid';
  const paymentMode = lot?.paymentMode || draftData?.paymentMode || 'Cash';
...
⚖️ *Quantity:* ${sourceQuantity} ${sourceUnit} ${sourceBoxes > 0 ? `(${sourceBoxes} boxes)` : ''}
                ...
                <table className="w-full text-left text-xs border-collapse border border-gray-300">
                  <thead>
                    <tr className="bg-gray-100 text-black border-b border-gray-300">
                      ...
                      <th className="p-2 font-bold text-center">Boxes</th>
                    ...
                    {linkedShipment && linkedShipment.items.length > 0 ? (
                      linkedShipment.items.map((item, idx) => (
                        <tr key={idx} className="border-b border-gray-200">
                          ...
                          <td className="p-2 text-center font-mono">{item.boxesCount || '—'}</td>
                      ...
                      <tr className="border-b border-gray-200">
                        ...
                        <td className="p-2 text-center font-mono">{sourceBoxes}</td>
                        <td className="p-2 text-right font-mono font-bold">
        ...
        <div className="no-print bg-white border-t border-[#E8E2D9] p-3 sm:p-4 flex flex-wrap items-center justify-between gap-2.5">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-[#FCFBF9] border border-[#2E6349] text-[#2E6349] font-bold text-xs hover:bg-[#E9F3EE] transition flex items-center gap-1.5 shadow-2xs cursor-pointer"
            >
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-[#2E6349] text-white font-black text-xs hover:bg-[#1F4532] transition flex items-center gap-1.5 shadow-xs cursor-pointer"
            >
